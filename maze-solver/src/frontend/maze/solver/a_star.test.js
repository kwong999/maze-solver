...
describe("nextMove", () => {
  ...
  AStar.nextMove(board, 4, "");
  test("will update usedMove list", () => {
    expect(mCodeToPos).toHaveBeenCalledTimes(4);
...
describe("determineNextPosition", () => {
  test("return false when possibleMove is empty", () => {
    ...
    board.possibleMove = [];
    expect(AStar.determineNextPosition(board)).toBeFalsy();
  });
  ...
  test("return possible move with lowest FScore", () => {
    ...
    expect(AStar.determineNextPosition(board)).toEqual(4);
  });
});
...
describe("GScore", () => {
  ...
  test("calculate GScore", () => {
    expect(AStar.GScore([-1, 0], tileC1)).toEqual(10);
    expect(AStar.GScore([1, 0], tileC2)).toEqual(10);
    expect(AStar.GScore([0, -1], tileC3)).toEqual(10);
    expect(AStar.GScore([0, 1], tileC4)).toEqual(10);
    expect(AStar.GScore([0, -1], tileCC1)).toEqual(20);
  });
});

describe("HScore", () => {
  const tile1 = new Tile([1, 1]);
  test("calculate HScore", () => {
    expect(AStar.HScore([0, 1], tile1)).toEqual(10);
    expect(AStar.HScore([2, 1], tile1)).toEqual(10);
    expect(AStar.HScore([1, 0], tile1)).toEqual(10);
    expect(AStar.HScore([1, 2], tile1)).toEqual(10);
    expect(AStar.HScore([2, 2], tile1)).toEqual(14);
    expect(AStar.HScore([3, 2], tile1)).toEqual(24);
  });
});